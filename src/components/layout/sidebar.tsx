...
export function Sidebar() {
  ...
  // preserva cliente/período ao navegar entre páginas
    ...
    <aside className="bg-board fixed inset-y-0 left-0 z-30 hidden w-64 flex-col bg-bispo-green text-white/90 lg:flex">
      <div className="flex items-center gap-2.5 px-5 py-6">
        ...
        <div className="leading-tight">
          ...
          <p className="text-[11px] font-medium uppercase tracking-widest text-white/55">Analytics</p>
        </div>
      </div>
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                active ? "bg-white text-bispo-green shadow-sm" : "text-white/75 hover:bg-white/10 hover:text-white",
              )}
            >
              <Icon className="h-[18px] w-[18px]" strokeWidth={2.2} />
              <span>{label}</span>
              {highlight && !active && <span className="ml-auto h-1.5 w-1.5 rounded-full bg-white/70" />}
            </Link>
          );
        })}
      </nav>
      ...
      <div className="mt-auto px-3 pb-4">
        ...
        >
          <SETTINGS_ITEM.icon className="h-[18px] w-[18px]" strokeWidth={2.2} />
          {SETTINGS_ITEM.label}
        </Link>
